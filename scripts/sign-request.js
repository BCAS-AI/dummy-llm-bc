...
// formulas from the BCA OAuth & Signature documentation.
//
// Usage:
...
function printLlm(accessToken, bodyJson) {
  if (!accessToken) {
    console.error('Missing <accessToken>. Run `node scripts/sign-request.js oauth` first and call the endpoint to obtain one.');
    process.exit(1);
  }
