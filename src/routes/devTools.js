...
// signed X-TIMESTAMP/X-SIGNATURE pair instead. Not part of the mirrored BCA
// API surface.
...
const PRIVATE_KEY_PATH = path.join(__dirname, '..', '..', 'dist', 'keys', 'test-client-private-key.pem');
...
router.get('/sign/oauth', (req, res) => {
  ...
  try {
    ...
    return res.status(500).json({
      error: 'Test private key not found. Run `npm install` (the postinstall step copies keys/ into dist/keys/).',
    });
  }
  ...
  return res.status(200).json({ clientId: client.clientId, timestamp, signature });
});
